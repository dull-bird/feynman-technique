...
const segments = JSON.parse(
  fs.readFileSync(path.join(__dirname, "segments12.json"), "utf8"),
);
const segmentsDir = path.join(__dirname, "segments12");
...
fs.writeFileSync(
  path.join(segmentsDir, "concat12.txt"),
  `${concatEntries.join("\n")}\n`,
);

const outWav = path.join(segmentsDir, "narration12_full.wav");
const outMp3 = path.join(publicDir, "narration.mp3");
execSync(
  `ffmpeg -y -f concat -safe 0 -i "${path.join(segmentsDir, "concat12.txt")}" -ar 44100 -ac 2 "${outWav}"`,
  { stdio: "pipe" },
);
