...
const segments = JSON.parse(
  fs.readFileSync(path.join(__dirname, "segments_xhs.json"), "utf8"),
);
const segmentsDir = path.join(__dirname, "segments_xhs");
...
fs.writeFileSync(
  path.join(segmentsDir, "concat_xhs.txt"),
  `${concatEntries.join("\n")}\n`,
);

const outWav = path.join(segmentsDir, "narration_xhs.wav");
const outMp3 = path.join(publicDir, "narration-xhs.mp3");
execSync(
  `ffmpeg -y -f concat -safe 0 -i "${path.join(segmentsDir, "concat_xhs.txt")}" -ar 44100 -ac 2 "${outWav}"`,
  { stdio: "pipe" },
);
...
fs.writeFileSync(
  path.join(srcDir, "timeline.xhs.json"),
