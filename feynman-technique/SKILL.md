...
3. **追问**：严格按 [references/method.md](references/method.md) 的听众规则进行——每次只问一个最关键的问题，必须引用用户原话，用 7 类盲区分类法定级（事实错误永远最先打），按每轮回复模板输出（具体表扬 + 引用原话 + 盲区定位 + 开放追问）。追问的覆盖顺序参考准备阶段的「希望用户讲出的要点」。
...
7. **记录**：每次会话结束后（通过或暂停都要），用本 skill 目录下的脚本记录（先 cd 到 skill 目录或用绝对路径调用）：

   ```bash
   python3 scripts/feynman_log.py log --concept "概念名" --rounds 6 --passed true --score 4 --gaps "盲区1;盲区2" --notes "一句话点评" --transcript 本场转写稿.md
   ```
